...
var $author$project$Main$GiveMeTimeZone = function (a) {
	return {$: 4, a: a};
};
var $author$project$Main$Millis = 0;
var $author$project$Main$Model = F4(
	function (ts, unit, zone, result) {
		return {J: result, F: ts, x: unit, N: zone};
	});
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		A4($author$project$Main$Model, 0, 0, $elm$time$Time$utc, ''),
		A2($elm$core$Task$perform, $author$project$Main$GiveMeTimeZone, $elm$time$Time$here));
};
...
var $author$project$Main$Sec = 1;
var $danhandrea$elm_date_format$DateFormat$English = 0;
...
var $danhandrea$elm_date_format$DateFormat$format = F3(
	function (fmt, zone, time) {
		return A4($danhandrea$elm_date_format$DateFormat$formatI18n, 0, fmt, zone, time);
	});
...
var $author$project$Main$update = F2(
	function (msg, model) {
		var toPosix = F2(
			function (unit, ts) {
				return (!unit) ? $elm$time$Time$millisToPosix(ts) : $elm$time$Time$millisToPosix(ts * 1000);
			});
		var pattern = 'ddd, dd/MM/yyyy HH:mm:ss';
		var formatter = A3(
			$danhandrea$elm_date_format$DateFormat$format,
			pattern,
			model.N,
			A2(toPosix, model.x, model.F));
		switch (msg.$) {
			case 0:
				var ts = msg.a;
				return _Utils_Tuple2(
...
var $author$project$Main$Convert = {$: 3};
var $author$project$Main$MillisUnit = function (a) {
	return {$: 1, a: a};
};
var $author$project$Main$SecUnit = function (a) {
	return {$: 2, a: a};
};
var $author$project$Main$SetTs = function (a) {
	return {$: 0, a: a};
};
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Events$onCheck(ev)
						]),
					_List_Nil)
				]));
	});
...
var $author$project$Main$view = function (model) {
	var secChecked = model.x === 1;
	var millisChecked = !model.x;
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Events$onInput($author$project$Main$SetTs)
					]),
				_List_Nil),
				A2(
				...
				_List_fromArray(
					[
						A3($author$project$Main$check, 'millis', millisChecked, $author$project$Main$MillisUnit),
						A3($author$project$Main$check, 'sec', secChecked, $author$project$Main$SecUnit)
					])),
				A2(
				$elm$html$Html$div,
				_List_Nil,
